...
let printf style = Printf.ksprintf (print_string style)
...
external win_show_cursor : unit -> unit = "ANSITerminal_ShowCursor"
external win_hide_cursor : unit -> unit = "ANSITerminal_HideCursor"

let set_cursor x y =
  if is_out_channel_atty stdout then
    let x0, y0 = pos_cursor () in
    let x = if x <= 0 then x0 else x and y = if y <= 0 then y0 else y in
...
let move_cursor dx dy =
  if is_out_channel_atty stdout then
    ...
    let x = if x <= 0 then 1 else x and y = if y <= 0 then 1 else y in
    set_cursor_ x y (* FIXME: (x,y) outside the console?? *)
...
let show_cursor () = if is_out_channel_atty stdout then win_show_cursor ()
let hide_cursor () = if is_out_channel_atty stdout then win_hide_cursor ()
