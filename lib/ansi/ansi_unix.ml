(* File: ANSITerminal_unix.ml Allow colors, cursor movements, erasing,... under
   Unix shells.
   *********************************************************************
...
let set_cursor x y =
  if is_out_channel_atty stdout then
    ...
    else if (* x > 0 *) y <= 0 then printf "\027[%iG%!" x
...
let send_and_read_response fdin query fmt f =
  ...
  Unix.tcsetattr fdin Unix.TCSANOW
    {
      tty with
      Unix.c_ignbrk = false;
      c_brkint = false;
...
let pos_cursor () =
  if is_out_channel_atty stdout then
    try
      send_and_read_response Unix.stdin pos_cursor_query "\027[%d;%dR"
        (fun y x -> (x, y))
...
let size () =
  if !isatty Unix.stdin then size_ Unix.stdin
