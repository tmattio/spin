...
let user_interrupt () =
  if !exit_on_user_interrupt then (
    ...
    flush stderr;
    exit 130)
  else raise Exn.Interrupted_by_user
...
let print_err ?(style = Style.default) err =
  let () =
    ...
    | _ ->
      prerr_string
        (Ansi.sprintf style.Style.error_format "%s %s" style.Style.error_icon
           err)
  in
  flush stderr
...
let with_raw ?(hide_cursor = false) ?(when_ = Unix.TCSANOW) fd f =
  if Unix.isatty fd then (
    ...
    | e ->
      Unix.tcsetattr fd Unix.TCSADRAIN term_init;
      if hide_cursor then Ansi.show_cursor ();
      raise e)
  else f ()
