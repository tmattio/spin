...
set_filename() {
  ...
  if [ "$OS" == "Linux" ]; then
    FILENAME="spin-linux-x64"
  elif [ "$OS" == "Darwin" ]; then
    FILENAME="spin-darwin-x64"
  else
    echo "OS $OS is not supported."
    echo "If you think that's a bug - please file an issue to https://github.com/tmattio/spin/issues"
    exit 1
  fi
}
...
download_spin() {
  ...
  mv "$DOWNLOAD_DIR/main.exe" "$INSTALL_DIR/spin"
